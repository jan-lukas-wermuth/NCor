  ...
  if (nominal == "r"){ # case 1: Rows are nominal
    ...
    if (isTRUE(Test)){
      ...
      if (isTRUE(CIs)){
        ...
        cases <- rstatix::counts_to_cases(ContTable)
        finalres_CIs <- RCor::RCor(as.numeric(as.vector.factor(cases[,1])), as.numeric(as.vector.factor(cases[,2])), method = "gamma", Fisher = FALSE)[1:3]
      ...
      if (isTRUE(CIs)){
        ...
        rownames(ContTable) <- final_perm_row
        cases <- rstatix::counts_to_cases(ContTable)
        finalres <- RCor::RCor(as.numeric(as.vector.factor(cases[,1])), as.numeric(as.vector.factor(cases[,2])), method = "gamma", Fisher = FALSE)[1:3]
